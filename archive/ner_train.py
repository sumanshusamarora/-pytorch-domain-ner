...
class EntityExtraction(nn.Module):
    def __init__(
        ...
        if self.use_char_embedding:
            self.char_embed = nn.Embedding(
                num_embeddings=x_char_encoder.vocab_size,
            ...
            self.cnn_seq = nn.Sequential(
                nn.Conv1d(
                    ...
                    kernel_size=self.maxp_size,
                    padding=1,
                ),
                nn.ReLU(),
                nn.MaxPool1d(kernel_size=self.maxp_size),
            )

            self.conv_max_pool_char = TimeDistributed(self.cnn_seq, batch_first=True)

        # LSTM for concatenated input
        self.lstm_ner = nn.LSTM(
            input_size=256,  # self.word_embed_dim+self.tag_embed_dim,
    ...
    def forward(self, x_word, x_pos, x_char, mask, y_word=None, train=True):
        ...
        if self.use_char_embedding:
            ...
            )  # Flatten the dim * chars

            concat = torch.cat((concat, char_out), dim=2)
        ...
        for k, data_test in enumerate(self.dataloader_test):
            with torch.no_grad():
                ...
                _ = [
                    ...
                    for out in np.where(
                        test_ner_truth_result == 0, 9, test_ner_truth_result
                    )
                ]
...
if __name__ == "__main__":
    ...
    with mlflow.start_run() as run:
        ...
        mlflow.log_param("COMMENT", "Added character embedding with CNN")
        ...
        mlflow.log_param("TAG_EMBED_DIM", TAG_EMBED_DIM)
        mlflow.log_param("CHAR_EMBED_DIM", CHAR_EMBED_DIM)
        mlflow.log_param("CNN_DIM", CNN_DIM)
        mlflow.log_param("MAXP_SIZE", MAXP_SIZE)
        mlflow.log_param("USE_CHAR_EMBED", USE_CHAR_EMBED)
        # Load Data
        ...
        (
            ...
            y_ner_list_train,
            y_ner_list_test,
        ) = split_test_train(X_text_list, X_tags, y_ner_list, split_size=TEST_SPLIT)
